...
def run_inference(
    *,
    ...
    """Evaluate action recognition for one or more models and save results/raw outputs."""
    ...
    for model_key in models:
        print(f"\n=== Evaluating model: {model_key} (action recognition) ===")
    ...
    print(f"\nSaved action recognition summary to {result_path}")
    print(f"Saved action recognition raw outputs to {raw_path}")
    release_all_models()


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run action recognition inference and save metrics/raw outputs."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Where to save metrics (defaults to results/action_recognition_result_size{size}.json).",
    )
    parser.add_argument(
        "--raw-path",
        type=Path,
        default=None,
        help="Where to save raw outputs (defaults to results/action_recognition_raw_size{size}.json).",
    )
...
def main() -> None:
    ...
    result_path = args.result_path or default_dir / f"action_recognition_result_size{args.size}.json"
    raw_path = args.raw_path or default_dir / f"action_recognition_raw_size{args.size}.json"
